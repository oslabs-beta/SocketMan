...
const detectSupportedFeatures = (io) => {
  //depending on version we have, we may or may not have certain methods.
...
const registerFeatureHandlers = (io, socket, supportedFeatures) => {
  //if the supported features array includes emit
  if (supportedFeatures.includes(typed_events_1.Feature.EMIT)) {
    //socket is now listening for emits on developer end
    socket.on('emit', (nsp, filter, ev, ...args) => {
      debug(`emit ${ev} to all socket instances in namespace ${nsp} and room ${filter}`);
      //namespace will always be a guarantee (if namespace isn't specified, will be '/')
      if (filter) {
        io.of(nsp)
          .in(filter)
          .emit(ev, ...args);
    ...
    if (typeof io.socketsJoin === 'function') {
      ...
      socket.on('join', (nsp, room, filter) => {
        if (filter) {
          ...
          //all sockets in filter(refers to current room) will join sockets in room(new room)
          io.of(nsp).in(filter).socketsJoin(room);
        }
        else {
          debug(`make all socket instances in namespace ${nsp} join room ${room}`);
          // all sockets in a given namespace join new room
...
const registerVerboseListeners = (adminNamespace, nsp) => {
  //socket.data - object that can be used to share info btw Socket.IO servers
  //https://socket.io/docs/v4/server-api/
  nsp.prependListener('connection', (socket) => {
    // @ts-ignore
    //gets id from socket client?
...
function instrument(io, opts) {
  ...
  //creates a namespace => io.of that allows us the connection to the io instance we've created in server when user invokes io.
  //https://socket.io/docs/v4/namespaces/
  const adminNamespace = io.of(options.namespaceName);
  //passing in 
  initAuthenticationMiddleware(adminNamespace, options);
  //supported features will be an array of strings, pertaining to functionality in io.
  const supportedFeatures = options.readonly ? [] : detectSupportedFeatures(io);
  //making sure supported features always has aggregated events
  supportedFeatures.push(typed_events_1.Feature.AGGREGATED_EVENTS);
  const isDevelopmentMode = options.mode === 'development';
  //if we are in development mode, adding all events to our supported features
  if (isDevelopmentMode) {
    supportedFeatures.push(typed_events_1.Feature.ALL_EVENTS);
  }
  debug('supported features: %j', supportedFeatures);
  //namespace created for our GUI tool, creating a new listening
